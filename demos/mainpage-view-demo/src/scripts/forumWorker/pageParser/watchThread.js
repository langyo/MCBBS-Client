...
for (let i = 0; i < postList.length; ++i) {
  ...
  if ((match = /^post_([0-9]+)$/.exec(postList[i].id))) {
    ...
    // 内容
    try {
      posts[match[1]].content = postList[i]
        .querySelectorAll("table > tbody > tr")[0]
        .querySelectorAll(
          "td.plc > div.pct > div.pcb > div.t_fsz > table > tbody > tr > td.t_f"
        )[0].innerHTML;
    } catch (e) {
      console.log(e);
    }
    // 层主
    try {
      posts[match[1]].author = postList[i]
        .querySelectorAll("table > tbody > tr")[0]
        .querySelectorAll("td.pls > div > div.pi > div > a")[0]
        .getAttribute("href")
        .match(/uid=([0-9]+)/)[1];
    } catch (e) {
      console.log(e);
    ...
    console.log(
      document
        .querySelectorAll(".rate")[0]
        .querySelectorAll(
          "dd > table > tbody:nth-child(1) > tr > th:nth-child(1) > a"
        )[0]
        .getAttribute("href")
    );
  }
...
try {
  let n = document
    .querySelectorAll("#postlist > table")[0]
    .querySelectorAll("tbody > tr > td.plc.ptm.pbn.vwthd > span > img");
  for (let i of n) {
...
if (document.querySelectorAll("#tath > a").length > 0) {
  ...
  try {
    thread.author = document
      .querySelectorAll("#postlist > div")[0]
      .querySelectorAll("table > tbody > tr")[0]
      .querySelectorAll("td.pls > div > div.pi > div > a")[0]
      .getAttribute("href")
      .match(/uid=([0-9]+)/)[1];
