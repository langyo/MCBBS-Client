...
for (let i of document.querySelectorAll(
  "#portal_block_800_content > div.slidebox > div.slideshow > li"
)) {
  let n = {
...
for (let i of document.querySelectorAll(
  "#ct > div.mn > div.fl.bm > div.bm.bmw.cl"
)) {
  ...
  if (/flg/.test(i.className)) {
    ...
    for (let j of i.querySelectorAll(
      "div.bm_c.forum_index_bm > table > tbody > tr > td"
    )) {
      try {
        let n = {
          name: j.querySelector("dl > dt > a").innerText.trim(),
          id: /^forum\-([A-Za-z0-9\-]+)\.html$/
            .exec(j.querySelector("dl > dt > a").getAttribute("href"))[1]
            .trim(),
          avatar: j
    ...
    // 单栏
    for (let j of i.querySelectorAll(
      "div.bm_c.forum_index_bm > table > tbody > tr"
    )) {
      try {
        let n = {
          name: j.querySelector("td > h2 > a").innerText.trim(),
          id: /^forum\-([A-Za-z0-9\-]+)\.html$/
            .exec(j.querySelector("td > h2 > a").getAttribute("href"))[1]
            .trim(),
          avatar: j
            .querySelector("td.fl_icn > a > img")
            .getAttribute("src")
            .trim()
        };
        forums.push(n);
...
for (let i of document.querySelectorAll(
  "#portal_block_807_content > div > ul > li"
)) {
  mainPage.headThreads.latestThread.push({
    title: i.querySelectorAll("a")[1].innerText.trim(),
    href: /thread-([0-9]+)-/.exec(
      i.querySelectorAll("a")[1].getAttribute("href")
    )[1],
    author: /uid=([0-9]+)/.exec(
      i.querySelector("em > a").getAttribute("href")
    )[1]
  });
}
for (let i of document.querySelectorAll(
  "#portal_block_806_content > div > ul > li"
)) {
...
for (let i of document.querySelectorAll(
  "#portal_block_809_content > div > ul > li"
)) {
