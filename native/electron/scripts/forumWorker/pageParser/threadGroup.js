...
for (let i of document.querySelectorAll("#threadlist > div.bm_c > table > tbody > tr")) {
  ...
  if (i.querySelector('th > a.xst').style.fontWeight === 'bold') info.states.titleBold = true; // 帖子分级解析
...
let hrefType = /view=([a-zA-Z]+)/.exec(location.href)[1]; // 根据 URL 类型对 mainPage.headThreads 写入
